...
type: INT				{$$ = Type_node::createTypeNode(INT_TYPE);}
	...
	| CLASS_NAME '*'	{$$ = Type_node::createClassTypeNode(CLASS_NAME_TYPE, $1);}
    ;

// ---------- Константы ----------
numeric_constant: FLOAT_CONSTANT	{$$ = Numeric_constant_node::createFloatConstantNode($1);}
                | INTEGER_CONSTANT	{$$ = Numeric_constant_node::createIntegerConstantNode($1);}
                ;
...
declaration_list: declaration 					{$$ = Declaration_list_node::createDeclarationListNode($1);}
				| declaration_list declaration	{$$ = Declaration_list_node::addDeclarationListNode($1, $2);}
				;
...
init_declarator_list: init_declarator							{$$ = Init_declarator_list_node::createInitDeclaratorListNode($1);}
					| init_declarator_list ',' init_declarator	{$$ = Init_declarator_list_node::addInitDeclaratorListNode($1, $3);}
					;
...
parameter_list: parameter_declaration						{$$ = Parameter_list_node::createParameterListNode($1);}
			  | parameter_list ',' parameter_declaration	{$$ = Parameter_list_node::addParameterListNode($1, $3);}
			  ;
...
expression: IDENTIFIER							{$$ = Expression_node::createIdentifierExpressionNode($1);}
		  | literal								{$$ = Expression_node::createLiteralExpressionNode($1);}
		  | numeric_constant					{$$ = Expression_node::createNumericConstantExpressionNode($1);}
		  | '(' expression ')'					{$$ = Expression_node::createSimpleExpressionNode(PRIORITY_EXPRESSION_TYPE, $2);}
		  | SELF								{$$ = Expression_node::createSelfExpressionNode();}
		  | message_expression					{$$ = Expression_node::createSimpleExpressionNode(MESSAGE_EXPRESSION_EXPRESSION_TYPE, $1);}
		  | '-' expression %prec UMINUS			{$$ = Expression_node::createOperationExpressionNode(UMINUS_EXPRESSION_TYPE, NULL, $2);}
		  | '+' expression %prec UPLUS			{$$ = Expression_node::createOperationExpressionNode(UPLUS_EXPRESSION_TYPE, NULL, $2);}
		  | '&' expression %prec UAMPERSAND		{$$ = Expression_node::createOperationExpressionNode(UAMPERSAND_EXPRESSION_TYPE, NULL, $2);}
		  | expression '+' expression			{$$ = Expression_node::createOperationExpressionNode(PLUS_EXPRESSION_TYPE, $1, $3);}
		  | expression '-' expression			{$$ = Expression_node::createOperationExpressionNode(MINUS_EXPRESSION_TYPE, $1, $3);}
		  | expression '*' expression			{$$ = Expression_node::createOperationExpressionNode(MUL_EXPRESSION_TYPE, $1, $3);}
		  | expression '/' expression			{$$ = Expression_node::createOperationExpressionNode(DIV_EXPRESSION_TYPE, $1, $3);}
		  | expression EQUAL expression			{$$ = Expression_node::createOperationExpressionNode(EQUAL_EXPRESSION_TYPE, $1, $3);}
		  | expression NOT_EQUAL expression		{$$ = Expression_node::createOperationExpressionNode(NOT_EQUAL_EXPRESSION_TYPE, $1, $3);}
		  | expression '>' expression			{$$ = Expression_node::createOperationExpressionNode(GREATER_EXPRESSION_TYPE, $1, $3);}
		  | expression '<' expression			{$$ = Expression_node::createOperationExpressionNode(LESS_EXPRESSION_TYPE, $1, $3);}
		  | expression LESS_EQUAL expression	{$$ = Expression_node::createOperationExpressionNode(LESS_EQUAL_EXPRESSION_TYPE, $1, $3);}
		  | expression GREATER_EQUAL expression	{$$ = Expression_node::createOperationExpressionNode(GREATER_EQUAL_EXPRESSION_TYPE, $1, $3);}
		  | expression '=' expression			{$$ = Expression_node::createOperationExpressionNode(ASSIGNMENT_EXPRESSION_TYPE, $1, $3);}
		  ;
...
message_selector: IDENTIFIER				{$$ = Message_selector_node::createMethodSelectorNode($1, NULL);}
				| keyword_argument_list		{$$ = Message_selector_node::createMethodSelectorNode(NULL, $1);}
				;

keyword_argument_list: keyword_argument							{$$ = Keyword_argument_list_node::createKeywordArgumentListNode($1);}
					 | keyword_argument_list keyword_argument	{$$ = Keyword_argument_list_node::addKeywordArgumentListNode($1, $2);}
					 ;
...
for_statement: FOR '(' expression_e ';' expression_e ';' expression_e ')' statement		{$$ = For_statement_node::createForStatementNode($3, $5, $7, $9);}
			 | FOR '(' IDENTIFIER IN expression ')' statement							{$$ = For_statement_node::createForeachStatementNode(FOREACH_FOR_TYPE, NULL, $3, $5, $7);}
			 | FOR '(' type IDENTIFIER IN expression ')' statement						{$$ = For_statement_node::createForeachStatementNode(FOREACH_WITH_DECLARATION_FOR_TYPE, $3, $4, $6, $8);}
			 ;

// ---------- Операторы ----------

statement: ';'							{$$ = Statement_node::createSimpleStatementNode(EMPTY_STATEMENT_TYPE, NULL);}
		 | expression ';'				{$$ = Statement_node::createSimpleStatementNode(SIMPLE_STATEMENT_TYPE, $1);}
		 | RETURN expression_e ';'		{$$ = Statement_node::createSimpleStatementNode(RETURN_STATEMENT_TYPE, $2);}
		 | if_statement					{$$ = Statement_node::createComplexStatementNode(IF_STATEMENT_TYPE, $1);}
		 | while_statement				{$$ = Statement_node::createComplexStatementNode(WHILE_STATEMENT_TYPE, $1);}
		 | do_while_statement			{$$ = Statement_node::createComplexStatementNode(DO_WHILE_STATEMENT_TYPE, $1);}
		 | for_statement				{$$ = Statement_node::createComplexStatementNode(FOR_STATEMENT_TYPE, $1);}
		 | compound_statement			{$$ = Statement_node::createComplexStatementNode(COMPOUND_STATEMENT_TYPE, $1);}
		 | declaration					{$$ = Statement_node::createDeclarationStatementNode($1);}
		 | class_declaration_list		{$$ = Statement_node::createClassDeclarationStatementNode($1);}
		 ;
...
statement_list: statement					{$$ = Statement_list_node::createStatementListNode($1);}
			  | statement_list statement	{$$ = Statement_list_node::addStatementListNode($1, $2);}
			  ;
...
class_statement: class_interface		{$$ = Class_statement_node::createInterfaceClassStatementNode($1);}
			   | class_implementation	{$$ = Class_statement_node::createImplementationClassStatementNode($1);}
			   ;

class_statement_list: class_statement							{$$ = Class_statement_list_node::createClassStatementListNode($1);}
					| class_statement_list class_statement		{$$ = Class_statement_list_node::addClassStatementListNode($1, $2);}
					;
...
class_list: IDENTIFIER					{$$ = Class_list_node::createClassListNode($1);}
		  | class_list ',' IDENTIFIER	{$$ = Class_list_node::addClassListNode($1, $3);}
		  ;

instance_variables: '{' declaration_list '}'	{$$ = Instance_variables_node::createInstanceVariablesNode($2);}
				   ;

interface_declaration_list: declaration										{$$ = Interface_declaration_list_node::createDeclarationInterfaceDeclarationListNode($1);}
						  | property										{$$ = Interface_declaration_list_node::createPropertyInterfaceDeclarationListNode($1);}
						  | method_declaration								{$$ = Interface_declaration_list_node::createMethodDeclarationInterfaceDeclarationListNode($1);}
						  | interface_declaration_list declaration			{$$ = Interface_declaration_list_node::addDeclarationInterfaceDeclarationListNode($1, $2);}
						  | interface_declaration_list method_declaration	{$$ = Interface_declaration_list_node::addMethodDeclarationInterfaceDeclarationListNode($1, $2);}
						  | interface_declaration_list property				{$$ = Interface_declaration_list_node::addPropertyInterfaceDeclarationListNode($1, $2);}
						  ;
...
implementation_definition_list: declaration											{$$ = Implementation_definition_list_node::createDeclarationImplementationDefinitionListNode($1);}
							  | property											{$$ = Implementation_definition_list_node::createPropertyImplementationDefinitionListNode($1);}
							  | method_definition									{$$ = Implementation_definition_list_node::createMethodDeclarationImplementationDefinitionListNode($1);}
							  | implementation_definition_list declaration			{$$ = Implementation_definition_list_node::addDeclarationImplementationDefinitionListNode($1, $2);}
							  | implementation_definition_list method_definition	{$$ = Implementation_definition_list_node::addMethodDeclarationImplementationDefinitionListNode($1, $2);}
							  | implementation_definition_list property				{$$ = Implementation_definition_list_node::addPropertyImplementationDefinitionListNode($1, $2);}
							  ;
...
keyword_selector: keyword_declaration						{$$ = Keyword_selector_node::createKeywordSelectorNode($1);}
				| keyword_selector keyword_declaration		{$$ = Keyword_selector_node::addKeywordSelectorNode($1, $2);}
				;
